...
# 服务器版本和配置
__version__ = "2.0.0"
__tag__ = "ip_geolocation_find_ip_location_and_ip_info/2.0.0"
...
if API_KEY:
    ...
    default_headers["X-RapidAPI-Host"] = "ip-geolocation-find-ip-location-and-ip-info.p.rapidapi.com"
else:
...
client = httpx.AsyncClient(
    base_url="https://ip-geolocation-find-ip-location-and-ip-info.p.rapidapi.com", 
    timeout=30.0
)
...
async def _add_rapidapi_headers(method, url, **kwargs):
    ...
    if API_KEY:
        ...
        kwargs['headers']['X-RapidAPI-Host'] = "ip-geolocation-find-ip-location-and-ip-info.p.rapidapi.com"
    else:
...
def main():
    """主入口点"""
    print(f"🚀 启动 Ip Geolocation Find Ip Location And Ip Info MCP 服务器")
    print(f"📦 版本: {__tag__}")
